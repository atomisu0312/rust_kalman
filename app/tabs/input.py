...
def render_input_tab(dimension, obs_dimension):
    ...
    # 状態遷移行列 A (dimension x dimension)
    ...
    # 観測行列 C (obs_dimension x dimension)
    ...
    st.subheader("3. システムノイズ共分散行列 Q_v")
    st.caption(f"サイズ: {dimension} × {dimension} (対称行列)")
    Q_v = create_matrix_input(dimension, dimension, key_prefix=SESSION_STATE_KEY_QV)
    
    # 観測ノイズ共分散行列 Q_v (obs_dimension x obs_dimension)
    st.subheader("4. 観測ノイズ共分散行列 Q_w")
    st.caption(f"サイズ: {obs_dimension} × {obs_dimension} (対称行列)")
    Q_w = create_matrix_input(obs_dimension, obs_dimension, key_prefix=SESSION_STATE_KEY_QW)
